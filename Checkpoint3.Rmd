...
Finaldata <- Finaldata %>% mutate(as.Date(Day))
```
...
ggplot(Finaldata, aes(x = per_capita_income, fill = StateColor)) + 
    geom_density(alpha = .8)+ ggtitle("Income per Capita") + xlab("$") + ylab("Density") +scale_fill_manual(values=c("blue", "purple", "red"))

#Trump Support 

plot_usmap(data = Finaldata, values = "percent_republican2016", color = "white") + 
  scale_fill_continuous(low = "sky blue", high = "black", name = "Trump Support", label = scales::comma) + 
  theme(legend.position = "right") + ggtitle("Percent of Residents that Identify as Asian") +
...
#Scale.fill color manual ??? so to go from green to red
weekbefore <- data.frame(Finaldata) %>% filter(Day <= as.character(as.Date("2020-03-17")))
