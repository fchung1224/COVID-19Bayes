...
##Model 6: 
StateColor + per_capita_income + (StateColor | State)
```{r, warning = FALSE, cache=TRUE}
set.seed(454)
Finaldata<-
  Finaldata%>%
...
model_6 <- stan_glmer(
  ChinaVirusInterest ~ StateColor + per_capita_income + (StateColor | State), #Gives us random intercepts
  data = Finaldata, family = gaussian, chains = 3, iter = 3000*2
)
```


```{r}
head(data.frame(summary(model_6)),-2)
```
...
StateColor + totalTestResults + (StateColor | State)
...
model_8 <- stan_glmer(
  ChinaVirusInterest ~ StateColor + totalTestResults + (StateColor | State), #Gives us random intercepts
  data = Finaldata, family = gaussian, chains = 4, iter = 3000*2
)
```
```{r}
head(data.frame(summary(model_8)),-2)
```
...
#Neg Rate
model_14 <- stan_glmer(
  ChinaVirusInterest ~ Day + (1 | State) + negative_rate,
...
>>>>>>> 9c603cf8990ef70927a9e975a3ea3861661c2de6
)
```
