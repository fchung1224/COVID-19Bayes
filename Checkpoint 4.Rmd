...
mcmc_dens_overlay(model_100) #How can we just get a snapshot of our model evals?
```
...
```{r}
### For predicting one case 
new_FL <- data.frame(Day = 7 , State = "FL") #Doesn't recognize state
...
pred_100 <- data.frame(y_new = pred_100[-1])

ggplot(pred_100, aes(x=y_new))+geom_density()
...
# For correlation check (type like model 2 from activity 19)
# Store the chains
model_100_df <- as.array(model_100) %>%
  melt %>%
  pivot_wider(names_from = parameters, values_from = value)

# Wrangle the chains
# Note the need for `...` around the sigma_sq_b
model_100_df <- model_100_df %>%
  mutate(sigma_sq_w = `sigma`, sigma_sq_b = `Sigma[State:(Intercept),(Intercept)]`) %>%
  mutate(correlation = (sigma_sq_b/(sigma_sq_w+sigma_sq_b)))

ggplot(model_100_df, aes(x = correlation)) +
  geom_density(alpha = 0.5)
```
...
```{r}
#set.seed(454)
my_data <- data.frame(StateColor = "blue", State = "FL", Day = 18336)
...
ChinaVirusInterest ~ StateColor + percent_asian + (Day | State)								
...
ChinaVirusInterest ~ StateColor +percent_asian + totalTestResults + (StateColor | State)								
	(Intercept)	4.27E+01	7.19E-02	3.00E+00	3.90E+01	4.27E+01	4.65E+01	1741
	StateColorpurple	-2.52E-01	9.18E-02	4.05E+00	-5.38E+00	-2.01E-01	4.85E+00	1949
	StateColorred	-3.82E+00	9.17E-02	3.77E+00	-8.63E+00	-3.87E+00	8.40E-01	1685
	percent_asian	-1.07E+01	5.48E-01	2.86E+01	-4.61E+01	-1.05E+01	2.60E+01	2720
	totalTestResults	3.17E-04	4.75E-06	2.78E-04	-4.82E-05	3.19E-04	6.61E-04	3409
