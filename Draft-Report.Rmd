...
#### FIX 
$$
Y_{ij}|\theta_i, \mu_, \beta_2,\beta_3 \overset{ind}{\sim} N(\beta_0+ \beta_1X_1 + \beta_2X_2 + \beta_3X_3,\sigma^2) \\
\theta|\mu, \sigma_b   \sim N(\mu,\sigma_b) \\
\mu \sim N(..., ...) \\ 
\beta_2 \sim N(..., ...) \\
\beta_3 \sim N(..., ...) \\
\beta_4 \sim N(..., ...) \\
...
The output from the table above shows that within our timeframe as day increases, the interest in the coronavirus also increases. State color=blue is the default intercept of the model and having state color=purple decreases the interept by 1.89 while in a red state the intercept decreases by 3.7 in the interest index. Two other variables fo interest show that percent white and the positive rate both increase interest on average yet neither variable is specifically significant in this model. 

```{r}
pp_check(model_2)
```

This pp-check is similar to our other models showing a following of the trend with a specific hump around 0. This is due to 0 receiving extra weight in our data set than other values we would expect to see. 
...
Finallly in our analysis of the model we see that the MAE of this model is 8.98 with a within-50 rate of 0.59 and within-95 ratet of 0.965 showing some relaive gains from the first model.
...
Our data set includes not only repeated measures on our response variable $Y$ for each state, it also includes corresponding observations for our preditor variables as well. In order to increase the complexity of our model to include these aspects we will use the model strcuture below as a fundamental start point:

$$
\begin{split}
Y_{ij} | b_0, b_1, \beta_0, \beta_1, \sigma_w, \sigma_{0b}, \sigma_{1b} & \sim N(b_{0i} + b_{1i} X_{ij}, \; \sigma_w^2) \\
...
\end{split}
$$
With this structure our model will be able to account for the differing intercepts based on the day and also the differing slopes we expect to see by state. 
...
```
The output from the table above shows that within our timeframe as day increases, the interest in the China Virus term also increases. State color=blue is the default intercept of the model and having state color=purple decreases the interept by 1.02 while in a red state the intercept decreases by 3.6 in the interest index. Two other variables fo interest show that percent white and the positive rate both increase interest on average yet neither variable is specifically significant in this model. 
...
```
**Comment:** The assumption of using a Normal model is fairly reasonable besides the fact that we observe two bumps on both sides of the tails. We can assume these to be outliars. 
...
actual_FL_pred
```
To test the accuracy of the complex model we decided to test our prediction on one case too. We chose Florida as our test state and as you can see from the summaries above our predictions are
relatively close. The actual mean of interest in the term China Virus in Florida is 47.625 and our model predicted 46.10, this is only a 1.5 difference! 
